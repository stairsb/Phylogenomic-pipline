...
for msa_path in msa_list:
    ...
    for s in this_msa_seqs:
        ...
        assert ltp not in added_this_round, f"Multiple sequences corresponding to `{ltp}` in `{msa_path}`."
        assert len(s.string) == this_msa_len, f"Sequences in `{msa_path}` are not all the same length."
